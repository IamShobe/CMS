...
def bytes(t):
    return struct.pack(">"+("B"*len(t)), *t)

L2CAP_UUID = "0100"
AVRCP_UUID = 0x110e
BTSIG_ID = 0x1958  # BT Sig company ID
AVRCP_HEADER = bytes(
    (0x48, 0x00, 0x00, 0x19, 0x58))  # AVRCP specific commands header.
...
command_types[
    AVRCP_CTYPE_RESPONSE_NOT_IMPLEMENTED] = "RESPONSE_NOT_IMPLEMENTED "
...
event_ids = {}
event_ids[
    AVRCP_NOTIFICATION_EVENT_PLAYBACK_STATUS_CHANGED] = "PLAYBACK_STATUS_CHANGED"
...
event_ids[
    AVRCP_NOTIFICATION_EVENT_PLAYBACK_POS_CHANGED] = "PLAYBACK_POS_CHANGED"
event_ids[AVRCP_NOTIFICATION_EVENT_BATT_STATUS_CHANGED] = "BATT_STATUS_CHANGED"
event_ids[
    AVRCP_NOTIFICATION_EVENT_SYSTEM_STATUS_CHANGED] = "SYSTEM_STATUS_CHANGED"
event_ids[
    AVRCP_NOTIFICATION_EVENT_PLAYER_APPLICATION_SETTING_CHANGED] = "PLAYER_APPLICATION_SETTING_CHANGED"
event_ids[
    AVRCP_NOTIFICATION_EVENT_NOW_PLAYING_CONTENT_CHANGED] = "NOW_PLAYING_CONTENT_CHANGED"
event_ids[
    AVRCP_NOTIFICATION_EVENT_AVAILABLE_PLAYERS_CHANGED] = "AVAILABLE_PLAYERS_CHANGED"
event_ids[
    AVRCP_NOTIFICATION_EVENT_ADDRESSED_PLAYER_CHANGED] = "ADDRESSED_PLAYER_CHANGED"
...
def parseevents(payload):
    ...
    while (i < len(payload)):
        ...
        if (eventid > 0):
            ...
            result += "Event %02x %s %d " % (
            eventid, event_ids.get(eventid, "?"), eventval)
        i += 5
    return result
...
def parse_avrcp(packet):
    ...
    cr = (ptype >> 1) & 0x01
    ipid = (ptype & 0x01)
    print("Packet: label=%d type=%d c/r=%d ipid=%d %4x" % (
    transactionlabel, packet_type, cr, ipid, pid))
    try:
    ...
    print("Ctype=%02x (%s) pdu=%02x" % (
    ctype, command_types.get(ctype, "?"), pdu))
    if (pdu == 0x7c):  # Passthrough is actually an AVCTP command.
        ...
        if (action == AVRCP_OPERATION_ID_PLAY):
          ...
          4:9] == AVRCP_HEADER):  # All AVRCP commands have a PDU of 0x00, followed by the BTSIG_ID as company id
        ...
        if (pdu_ops == 0x10):  # Get Capability.
            capabilityid = payload[
                0]  # Can be 0x03 (supported events, or 0x02 Get Company IDs supported
            print("Get capability pdu=%02x capability id=%02x args=%d" % (
            pdu_ops, capabilityid, arglen))
            if (capabilityid == 3):
...
def respondevent(seq, payload):
    ...
    packet += struct.pack(">H", len(
        payload))  # At present just echoing the setting back.
    packet += payload
    socket.send(packet)
...
def requesteventvolume():
    ...
    packet += bytes((0x31, 0x00))  # 0x31 = Register Event
    payload = bytes(
        (AVRCP_NOTIFICATION_EVENT_VOLUME_CHANGED, 0x7f))  # 0x7f max volume.
